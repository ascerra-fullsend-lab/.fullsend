...
Read all `.txt` files in `$SCRIBE_NOTES_DIR`. If no files exist, write an empty result and stop.
...
The notes may be a rolling document with multiple meetings. Only extract from the MOST RECENT meeting section on or after `$SCRIBE_CUTOFF_DATE`. Look for date headers, timestamps, or structural cues. Ignore older content.
